...
| 命令                   | 功能        | 示例                                                        | 输出格式 |
| -------------------- | --------- | --------------------------------------------------------- | ---- |
| `data upload <file>` | 上传CSV数据文件 | `stock_backtest data upload a_stock.csv --symbol A_STOCK --format baostock` | JSON |
...
#### 数据格式参数说明
- `--format`：指定数据文件格式，支持以下选项：
  - `default`：默认格式，字段顺序为：日期,开盘价,最高价,最低价,收盘价,成交量
  - `baostock`：BaoStock格式，字段顺序为：date,code,open,high,low,close,preclose,volume,...

系统会根据数据格式自动选择对应的DataHandler实现（DataHandler或BaoStockDataHandler）。
...
#### CLI回调函数生命周期问题
- **问题**：回调函数捕获局部变量引用，导致回调执行时变量已超出作用域
- **修复方案**：使用CLI11的选项对象（`Option*`）替代局部变量引用，确保回调执行时能正确获取参数值
- **修复文件**：`backend/cli/CLI.cpp`

#### 数据库参数绑定类型检查
- **问题**：使用简单字符检查判断参数类型，不够可靠
- **修复方案**：使用`std::stoll`和`std::stod`进行类型转换，先尝试整数，再尝试浮点数，最后绑定为文本
- **修复文件**：`backend/models/Database.cpp`

#### 策略参数解析和空值检查
- **问题**：缺乏参数存在性验证和类型检查，可能导致运行时错误
- **修复方案**：添加完整的参数验证流程，包括参数存在性检查、JSON格式验证、参数类型验证和参数值有效性验证
- **修复文件**：`backend/cli/CLI.cpp`

### 14.2 代码质量改进

1. **异常处理增强**：完善异常捕获和错误处理机制
2. **空值检查**：添加对可能为null的字段的检查
3. **类型安全**：加强类型检查，避免类型转换错误
4. **参数验证**：增加参数有效性验证，提高系统健壮性
...
两种访问方式共享相同的后端核心逻辑，确保了功能的一致性和代码的可维护性，能够满足不同类型用户的需求。经过修复和改进，系统更加健壮，能够正确处理各种边界情况和错误输入，提高了系统的稳定性和可靠性。
